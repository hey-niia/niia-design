...
- **Fixed: strokes were getting cut off mid-gesture.** `onPointerLeave` was wired to end the stroke, so any fast/wide gesture that grazed just outside the canvas's rectangle (easy to do when drawing edge-to-edge) fired `pointerleave` and terminated the stroke early, well before the user actually lifted their finger/mouse — reported as "my drawing gets cut off by something on top when I draw side to side fully." Removed the `onPointerLeave` handler; `canvas.setPointerCapture()` (already called on `pointerdown`) keeps delivering `pointermove`/`pointerup` to the canvas regardless of where the pointer physically is, so a stroke now only ends on an actual `pointerup`/`pointercancel`, never on merely crossing the canvas boundary.
